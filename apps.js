const CONFIG = {
    BASE_URL: 'https://gasless-tisha-fluty.ngrok-free.dev//api',
...
const fetchWithFallback = async (endpointPattern) => {
    let res = await fetch(`${CONFIG.BASE_URL}/${CONFIG.LANG}${endpointPattern}`, REQUEST_OPTIONS);
    ...
    if (!res.ok || json.error || (json.data && Object.keys(json.data).length === 0)) {
        ...
        for (let lang of availableLangs) {
            ...
            try {
                const fallbackRes = await fetch(`${CONFIG.BASE_URL}/${lang}${endpointPattern}`, REQUEST_OPTIONS);
                if (fallbackRes.ok) {
                    const fallbackJson = await fallbackRes.json();
                    if (fallbackJson && !fallbackJson.error && fallbackJson.data) {
                        setLanguage(lang); 
                        return fallbackJson;
                    }
                }
        ...
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; background: #f0f2f5;">
            <div style="text-align: center; padding: 40px; background: white; border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); max-width: 400px; width: 90%;">
                ...
                <div style="display: grid; gap: 10px;">
                    <a href="https://t.me/dramaboss" target="_blank" style="text-decoration: none; padding: 12px; background: #0088cc; color: white; border-radius: 8px; font-weight: 600;">Hubungi Administrator</a>
...
const getNavbar = () => {
    ...
    return `
    <nav class="bg-slate-950/80 backdrop-blur-xl border-b border-slate-900 sticky top-0 z-50 p-3 md:p-4">
        <div class="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
            
            <a href="javascript:void(0)" onclick="navigateTo('/')" class="text-xl md:text-2xl font-extrabold tracking-tighter text-indigo-500">
                DRAMA<span class="text-white">BOSS</span>
            </a>

            <div class="flex items-center md:order-last">
                <select onchange="switchLanguage(this.value.split('|')[0], this.value.split('|')[1])" 
                        class="bg-slate-900 text-white text-[10px] md:text-xs border border-slate-700 rounded-lg px-2 py-1 outline-none">
                    ${languageOptions}
                </select>
            </div>
            ...
            <div class="w-full md:flex-1 md:max-w-md relative order-last md:order-none">
                <input type="text" id="searchInput" onkeyup="window.handleSearch(event)" 
                       ...
                       class="w-full bg-slate-900/50 text-white text-sm border border-slate-800 rounded-xl md:rounded-full px-4 py-2.5 md:py-2 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all">
                <button onclick="window.executeSearch()" class="absolute right-3 top-2.5 md:top-2 text-slate-500 hover:text-indigo-400">
...
const getSkeleton = (type) => {
    const shimmer = "shimmer rounded-2xl bg-slate-800";
    let content = "";
    ...
    if (type === 'home') {
        ...
        content = `
            <div class="w-full h-80 bg-slate-800 shimmer mb-6"></div>
            <div class="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2">
                    <div class="h-6 w-3/4 bg-slate-800 rounded mb-4 shimmer"></div>
                    <div class="h-4 w-full bg-slate-800 rounded mb-2 shimmer"></div>
                    <div class="h-4 w-full bg-slate-800 rounded mb-2 shimmer"></div>
                    <div class="grid grid-cols-4 md:grid-cols-8 gap-3 mt-10">
                        ${[1, 2, 3, 4, 5, 6, 7, 8].map(() => `<div class="h-12 bg-slate-800 rounded-xl shimmer"></div>`).join('')}
                    </div>
                </div>
            </div>`;
    } else {
        content = `
			<div class="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 aspect-video bg-slate-800 rounded-2xl shimmer"></div>
                <div class="space-y-4">
                    ${[1, 2, 3, 4].map(() => `<div class="h-20 bg-slate-800 rounded-xl shimmer"></div>`).join('')}
                </div>
            </div>`;
    }
...
const initApp = async () => {
	if (isBot()) {
		document.body.innerHTML = `
			<div style="height:100vh; display:flex; align-items:center; justify-content:center; background: radial-gradient(circle at top left, #1e293b, #0f172a); color:white; font-family: 'Inter', system-ui, sans-serif; margin:0;">
				<div style="text-align:center; padding: 2rem; border-radius: 1.5rem; background: rgba(255,255,255,0.03); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.1); max-width: 400px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5);">
					...
					<button onclick="window.location.reload()" style="margin-top:1.5rem; background:#3b82f6; color:white; border:none; padding: 0.75rem 1.5rem; border-radius: 0.75rem; font-weight:600; cursor:pointer; transition: all 0.2s;">Coba Lagi</button>
				</div>
			</div>`;
...
const router = async () => {
    ...
    try {
        ...
        contentArea.innerHTML = `<div class="p-20 text-center text-white"><p>Gagal memuat konten. Silakan muat ulang.</p></div>`;
    } finally {
    ...
    try {
        ...
        if (results.length === 0) {
            container.innerHTML = `
                <div class="py-40 text-center flex flex-col items-center justify-center">
                    ...
                    <h2 class="text-white text-xl font-bold">Drama "${query}" tidak ditemukan</h2>
                    <p class="text-slate-500 mt-2 max-w-xs">Coba gunakan kata kunci lain atau periksa ejaan judul drama.</p>
                    <button onclick="navigateTo('/')" class="mt-8 px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full text-sm font-bold transition-all transform active:scale-95">
                        Kembali ke Beranda
                    </button>
                </div>`;
            return;
        ...
        container.innerHTML = `
            <div class="max-w-7xl mx-auto p-4 min-h-screen">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b border-slate-900 pb-6">
                    <div>
                        <h2 class="text-2xl font-bold text-white">Hasil Pencarian</h2>
                        <p class="text-sm text-slate-500 mt-1">
                            Ditemukan <span class="text-indigo-400 font-bold">${results.length}</span> drama untuk <span class="italic">"${query}"</span>
                        </p>
                    </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-x-4 gap-y-8">
                    ${results.map(item => {
                        ...
                        <div class="cursor-pointer group" onclick="navigateToDetail('${item.bookId}', '${itemOrigin}')">
                            <div class="relative aspect-[3/4] overflow-hidden rounded-2xl bg-slate-900 mb-3 shadow-2xl">
                                <img src="${item.cover}" 
                                     alt="${item.bookName}"
                                     loading="lazy"
                                     class="w-full h-full object-cover group-hover:scale-110 transition duration-500"
                                     onerror="this.src='https://placehold.co/300x400/1e293b/white?text=No+Image'">
                                
                                <div class="absolute top-3 right-3 bg-indigo-600 px-2.5 py-1 rounded-md text-[9px] text-white font-black shadow-xl uppercase tracking-tighter z-10 border border-white/20 backdrop-blur-sm">
                                    ${itemOrigin}
                                </div>
                                ...
                                <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                                    <div class="text-[10px] text-white font-bold flex items-center gap-1">
                                        <i class="fa fa-star text-yellow-500"></i> ${item.score || '8.5'}
                                    </div>
                                </div>
                            </div>
                            
                            <p class="text-xs font-bold text-slate-200 line-clamp-2 group-hover:text-indigo-400 transition-colors leading-snug px-1">
                                ${item.bookName}
                            </p>
                        </div>`;
                    }).join('')}
                ...
                <div class="mt-20 py-10 text-center border-t border-slate-900">
                    <p class="text-[10px] text-slate-600 uppercase tracking-[0.2em]">End of Results</p>
                </div>
            </div>
        `;
        ...
        container.innerHTML = `<div class="py-40 text-center text-red-500 font-bold">Gagal Memuat Hasil Pencarian</div>`;
    }
}
		...
		<div class="max-w-7xl mx-auto p-4 fade-in space-y-12 pb-20">
            ...
            <section>
                <h2 class="text-xl font-extrabold mb-6 flex items-center gap-3">
                    <span class="w-1.5 h-7 bg-indigo-500 rounded-full shadow-[0_0_15px_rgba(99,102,241,0.5)]"></span> 
                    Sedang <span class="text-indigo-400">Trending</span>
                </h2>
                <div class="flex overflow-x-auto gap-5 hide-scrollbar pb-4">
                    ${(data.trending || []).map(item => `
                        <div class="min-w-[170px] group cursor-pointer" onclick="navigateTo('?detail=${item.bookId}')">
                            <div class="relative overflow-hidden rounded-2xl border border-slate-800 shadow-xl transition-all group-hover:border-indigo-500/50 group-hover:scale-[1.02]">
                                ...
                                <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-3">
                                    <div class="w-full py-2 bg-indigo-600 rounded-xl text-[10px] font-bold text-center text-white">DETAIL</div>
                                </div>
                            </div>
                            <h3 class="mt-3 text-sm font-bold truncate group-hover:text-indigo-400 transition-colors">${item.bookName}</h3>
                        </div>
                    `).join('')}
                </div>
            </section>
            ...
            <section>
                ...
                <div class="flex overflow-x-auto gap-6 hide-scrollbar pb-4">
                    ${(data.terbaru || []).map((item, index) => `
                        <div class="min-w-[280px] bg-slate-900/40 rounded-3xl p-4 border border-slate-800 flex gap-4 hover:border-amber-500/30 transition-all cursor-pointer group" onclick="navigateTo('?detail=${item.bookId}')">
            ...
            <!-- Banner Iklan -->
			<div id="container-ads-728x90" class="hidden ad-banner w-full h-24 rounded-xl mb-8 flex-col items-center justify-center border border-dashed border-slate-700 bg-slate-800/30">
            ...
            <section>
                <h2 class="text-xl font-extrabold mb-6 flex items-center gap-3">
                    <span class="w-1.5 h-7 bg-emerald-500 rounded-full shadow-[0_0_15px_rgba(16,185,129,0.5)]"></span> 
                    Paling <span class="text-emerald-400">Populer</span>
                </h2>
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-5">
                    ${(data.popular || []).map(item => `
                        <div class="cursor-pointer group" onclick="navigateTo('?detail=${item.bookId}')">
                            <div class="aspect-[3/4] overflow-hidden rounded-2xl border border-slate-800 mb-3 relative">
                                <img src="${item.cover}" loading="lazy" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                <div class="absolute top-2 right-2 px-2 py-0.5 bg-emerald-500 rounded-md text-[9px] font-black text-white shadow-lg">HD</div>
                            </div>
                            <h3 class="text-xs font-bold line-clamp-2 leading-snug group-hover:text-emerald-400">${item.bookName}</h3>
                        </div>
                    `).join('')}
                </div>
            </section>
...
async function renderDetail(bookId, container) {
	...
	let html = `
            <div class="relative w-full h-80">
                ...
                <div class="absolute inset-0 flex items-end p-6 bg-gradient-to-t from-slate-900">
                    <div class="flex gap-4 items-end max-w-7xl mx-auto w-full">
                        ...
                        <div class="flex-1">
                            <h1 class="text-2xl md:text-4xl font-black text-white">${drama.bookName}</h1>
                            <div class="flex gap-3 mt-2">
                                <span class="text-green-400 text-sm font-bold"><i class="fa fa-star"></i> ${drama.score || '8.5'}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2">
                    <div class="flex flex-wrap gap-2 mb-6">
                        ${drama.tags && Array.isArray(drama.tags) ? drama.tags.map(t => 
                            `<span class="bg-slate-800 px-4 py-1.5 rounded-full text-xs font-medium text-slate-300 border border-slate-700"># ${t}</span>`
                        ).join('') : ''}
                    </div>

                    <h3 class="text-xl font-bold mb-3">Sinopsis</h3>
                    <p class="text-slate-400 leading-relaxed mb-8 text-justify">${drama.introduction || 'Tidak ada sinopsis tersedia.'}</p>
                    
                    <div id="container-ads-728x90" class="ad-banner w-full h-24 rounded-xl mb-8 flex-col items-center justify-center border border-dashed border-slate-700 bg-slate-800/30" style="display: none;">
						<span class="text-[10px] text-slate-500 uppercase tracking-widest"></span>
						<div id="ads-728x90"></div>
					</div>
                        ...
                        ${episodes && Array.isArray(episodes) ? episodes.map((ep, index) => {
                            ...
                            return `
                                <button onclick="navigateTo('?watch=${bookId}/${id}')" 
                                    class="bg-slate-800 hover:bg-indigo-600 hover:scale-105 transform transition-all py-3 rounded-xl text-sm font-bold border border-slate-700 shadow-lg">
                                    ${displayNum}
                                </button>
                            `;
                ...
                <div class="space-y-6">
                    <div id="container-ads-320x250" class="hidden ad-banner w-full h-64 rounded-2xl flex-col items-center justify-center border border-slate-700 bg-slate-800/50">
							<div id="ads-320x250">
                    ...
                    <div id="recommendation-list" class="space-y-4">
                        ${recommendations.map(item => `
                            <div class="flex gap-3 group cursor-pointer" onclick="navigateTo('?detail=${item.bookId}')">
                                <div class="w-20 h-28 bg-slate-800 rounded-lg overflow-hidden flex-shrink-0 border border-slate-700">
                                    <img src="${item.cover}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300">
                                </div>
                                <div class="flex flex-col justify-center overflow-hidden">
                                    <h4 class="text-sm font-bold text-slate-200 group-hover:text-indigo-400 line-clamp-2 transition-colors">${item.bookName}</h4>
                                </div>
                            </div>
                        `).join('')}
                    </div>
        ...
        container.innerHTML = `<div class="p-20 text-center text-white">Gagal memuat detail drama.</div>`;
    }
}
...
async function renderWatch(bookId, chapterId, container) {
    ...
    try {
        ...
        container.innerHTML = `<div class="p-20 text-center text-white">Video tidak tersedia.</div>`;
    }
...
function renderTiktokStyle(videoUrl, drama, chapterId, container, videoData) {
    ...
    container.innerHTML = `
        <div id="tiktok-container" class="fixed inset-0 bg-black z-[60] overflow-y-scroll hide-scrollbar" style="scroll-snap-type: y mandatory; height: 100vh;">
            ...
            ${prevEpisode ? `
            <div class="snap-item w-full h-full flex flex-col items-center justify-center bg-slate-900 text-white p-10 text-center" 
            ...
            <div class="snap-item relative w-full h-full flex items-center justify-center bg-black" 
                ...
                <div class="absolute bottom-0 left-0 right-0 z-40 p-4 bg-gradient-to-t from-black/90 via-black/40 to-transparent">
                    <div class="flex items-center gap-3 mb-4 group">
                        ...
                        <div class="relative flex-1 h-1 bg-white/20 rounded-full cursor-pointer" id="progress-container" onclick="seekVideo(event)">
                            <div id="progress-bar" class="absolute top-0 left-0 h-full bg-indigo-500 rounded-full w-0"></div>
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-6">
                            <button onclick="togglePlayPause()" class="text-white hover:text-indigo-400">
                                <i id="play-btn-icon" class="fa fa-pause text-xl"></i>
                            </button>
                            <button onclick="changePlaybackSpeed()" class="text-white font-bold text-xs bg-white/10 px-2 py-1 rounded border border-white/20">
                                <span id="speed-text">1.0x</span>
                            </button>
                            <button onclick="toggleMute(event)" class="text-white hover:text-indigo-400">
                                <i id="mute-icon" class="fa fa-volume-up text-xl"></i>
                            </button>
                        </div>
                        <button onclick="toggleFullscreen()" class="text-white hover:text-indigo-400">
                ...
                <div class="absolute right-4 bottom-32 flex flex-col gap-6 text-center z-30">
                    <div onclick="shareContent()" class="flex flex-col items-center cursor-pointer group">
                        <div class="bg-slate-800/60 w-12 h-12 rounded-full flex items-center justify-center border border-white/10 active:scale-90 transition-transform">
                            <i class="fa fa-share text-white text-xl"></i>
                        </div>
                        <span class="text-[10px] text-white mt-1 font-medium">Share</span>
                    </div>
                </div>

                <div class="absolute left-4 bottom-24 right-20 z-30 text-white pointer-events-none">
            ...
            <div class="snap-item w-full h-full flex flex-col items-center justify-center bg-slate-900 text-white p-10 text-center" 
                 ...
                 <div class="animate-bounce">
                    <i class="fa fa-chevron-up text-indigo-500 text-3xl"></i>
                 </div>
                 <p class="text-sm font-bold text-slate-400 mt-4">Lepas untuk Episode ${currentIndex + 2}</p>
            </div>` : ''}
        </div>
    `;
    ...
    container.innerHTML = `
        <div class="p-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 fade-in">
            <div class="lg:col-span-2 text-white">
                ...
                <div class="flex justify-between items-center mt-4 border-b border-slate-800 pb-4">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center shadow-lg shadow-indigo-500/20">
                    ...
                    <div class="flex gap-2">
                        <button class="bg-slate-800 hover:bg-slate-700 px-4 py-2 rounded-full text-sm transition active:scale-95">
                            <i class="fa fa-thumbs-up mr-1"></i> Like
                        </button>
                        <button class="bg-indigo-600 hover:bg-indigo-700 px-6 py-2 rounded-full text-sm font-bold transition active:scale-95 shadow-lg shadow-indigo-500/20">
                            Subscribe
                        </button>
                    </div>
                </div>
				...
				<!-- Banner Iklan -->
				<div id="container-ads-728x90" class="hidden mt-6 w-full h-24 bg-gradient-to-r from-slate-900 to-slate-800 rounded-xl border border-slate-700 items-center justify-between px-6 group cursor-pointer hover:border-indigo-500/50 transition-all shadow-lg">
					<div id="ads-728x90"></div>
				</div>

                <div class="mt-4 p-4 bg-slate-800/50 rounded-xl text-sm text-slate-300 border border-slate-700">
            ...
            <div class="space-y-4">
                <div id="container-320x250" onclick="showPopupAd()" class="hidden ad-banner h-32 rounded-xl border border-dashed border-slate-700 flex-col items-center justify-center text-slate-500 text-xs uppercase font-bold tracking-widest cursor-pointer hover:bg-slate-800/30 transition-colors">
					<span id="ads-320x250"></span>
				</div>
                
                <h3 class="font-bold text-white flex items-center gap-2">
                    <span class="w-1 h-5 bg-indigo-500 rounded"></span> Episode Lainnya
                </h3>
                ...
                <div class="space-y-3 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                    ${drama.chapterList && Array.isArray(drama.chapterList) ? drama.chapterList.map((ch, idx) => {
                        ...
                        const epNum = idx + 1;
                        const isActive = epId == chapterId ? 'border-indigo-500 bg-indigo-500/20 shadow-[0_0_15px_rgba(99,102,241,0.1)]' : 'border-slate-800 bg-slate-800/40 hover:bg-slate-800';
                        ...
                        return `
                            <div onclick="navigateTo('?watch=${bookId}/${epId}')" class="flex gap-3 cursor-pointer group p-2 rounded-xl border transition-all duration-300 ${isActive}">
                                <div class="flex flex-col justify-center min-w-0">
                                    <p class="text-sm font-bold text-slate-200 line-clamp-1 group-hover:text-indigo-400 transition-colors">
    ...
    try {
        ...
        container.innerHTML = `
            <div class="max-w-6xl mx-auto p-4 md:p-10 fade-in">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 border-b border-slate-800 pb-6">
                    <div>
                        <h1 class="text-3xl font-black text-indigo-500 uppercase tracking-tighter">${fileName}</h1>
                    </div>
                </div>
                
                <div class="bg-slate-950 border border-slate-900 rounded-3xl p-6 shadow-2xl overflow-hidden ring-1 ring-slate-800">
                    <pre class="text-indigo-300 text-[10px] md:text-sm font-mono leading-relaxed overflow-x-auto whitespace-pre-wrap">${escapeXml(xmlText)}</pre>
                </div>
            </div>
        `;
    } catch (err) {
        container.innerHTML = `<div class="p-20 text-center text-red-500 font-bold">${err.message}</div>`;
    }
}
